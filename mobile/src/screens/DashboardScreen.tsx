...
const ringColor = (s: number) => s >= 80 ? '#22d3ee' : s >= 60 ? '#a78bfa' : s >= 40 ? '#fb923c' : '#f87171';

function Ring({ score, size = 160 }: { score: number; size?: number }) {
  const anim = useRef(new Animated.Value(0)).current;
  const c = ringColor(score), stroke = size * 0.075;
  useEffect(() => { Animated.timing(anim, { toValue: score, duration: 1200, useNativeDriver: false }).start(); }, [score]);
  const label = score >= 80 ? 'Thriving' : score >= 60 ? 'Solid' : score >= 40 ? 'Getting By' : 'Struggling';
  return (
    <View style={{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }}>
      <View style={{ position: 'absolute', alignItems: 'center', justifyContent: 'center', width: size, height: size }}>
        ...
        <Text style={{ fontSize: size * 0.1, color: '#64748b', fontWeight: '600', marginTop: 2 }}>{label}</Text>
      </View>
...
function Card({ icon, value, unit, goal, col, sub }: { icon: string; value: any; unit: string; goal?: number; col: string; sub?: string }) {
  ...
  return (
    <View style={S.card}>
      ...
      {p !== null && <View style={S.bar}><View style={[S.fill, { width: (p*100)+'%' as any, backgroundColor: col }]} /></View>}
    </View>
  );
}
      ...
      <ScrollView contentContainerStyle={S.scroll} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={async()=>{setRefreshing(true);await sync(userId);setRefreshing(false);}} tintColor="#a78bfa" />} showsVerticalScrollIndicator={false}>
        ...
        <View style={S.scoreCard}>
          ...
          <View style={S.breakdown}>
            {[
              {l:'Sleep',v:Math.min(100,Math.round((h.sleepHours/8)*100)),c:'#818cf8'},
              {l:'Fitness',v:Math.min(100,Math.round((h.steps/10000)*100)),c:'#34d399'},
              {l:'Heart',v:Math.min(100,Math.max(0,100-Math.abs(h.heartRate-65))),c:'#f472b6'},
              {l:'Calories',v:Math.min(100,Math.round((h.activeCalories/500)*100)),c:'#fbbf24'},
              {l:'Activity',v:Math.min(100,Math.round((h.standHours/12)*100)),c:'#22d3ee'},
            ].map(i=>(<View key={i.l} style={{alignItems:'center'}}><Text style={{color:i.c,fontSize:15,fontWeight:'800'}}>{i.v}</Text><Text style={{color:'#475569',fontSize:10,fontWeight:'600',marginTop:2}}>{i.l.toUpperCase()}</Text></View>))}
          </View>
        </View>
